...
export default {
  ...
  async findByDistrict(req: Request, res: Response) {
    /**
     #swagger.tags = ['Regions']
     **/
    try {
      const { name } = req.query;
      const result = await RegionModel.findByDistrict(`${name}`);
      response.success(res, result, "success get region by district name");
    } catch (error) {
      response.error(res, error, "failed to get region by district name");
    }
  },
  async findByVillage(req: Request, res: Response) {
    /**
     #swagger.tags = ['Regions']
     **/
    try {
      const { name } = req.query;
      const result = await RegionModel.findByVillage(`${name}`);
      response.success(res, result, "success get region by village name");
    } catch (error) {
      response.error(res, error, "failed to get region by village name");
    }
  },
